...
sudo mkinitcpio -g /boot/initrd.img-6.1.6 -k 6.1.6
sudo mkimage -A arm -O linux -T ramdisk -C none -a 0 -e 0 -n uInitrd -d /boot/initrd.img-6.1.6 /boot/uInitrd-6.1.6
sudo rm -f /boot/dtb /boot/Image /boot/uInitrd
sudo ln -sf dtb-6.1.6 /boot/dtb
sudo ln -sf vmlinuz-6.1.6 /boot/Image
sudo ln -sf uInitrd-6.1.6 /boot/uInitrd
...
cd linux
wget -O .config https://github.com/heiher/kernel-nanopi-m4/raw/master/boot/config-6.1.6
